// https://www.openprocessing.org/sketch/256649 Bézier curve subdivision demo
// by Raven Kwok (aka Guo, Ruiwen) | http://ravenkwok.com/ | raystain@gmail.com
// Originally created as one of the generative systems for the project <Build the Cities> | http://ravenkwok.com/build-the-cities/
Cell rootCell;
  ...
  int[] ctlIndices() {
    ...
    if (sLyr == eLyr && sRow == eRow) {
      if (boolean(round(random(1)))) {
        result = new int[] {
          (sLyr + 2) % 4, sRow, sCol, (eLyr + 2) % 4, eRow, eCol
        };
      } else {
        result = new int[] {
          sLyr,
      ...
      if (boolean(round(random(1)))) {
        result = new int[] {
          (sLyr + 2) % 4, sRow, sCol, (sLyr + 2) % 4, sRow, sCol
        };
      } else {
        result = new int[] {
          sLyr,
